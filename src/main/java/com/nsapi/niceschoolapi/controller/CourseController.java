...
public class CourseController {
    ...
    @RequestMapping("sel")
    @ResponseBody
   public Object sel(Integer page, Integer limit, CourseVO CourseVO, String cname){

        CourseVO.setCname(cname);
        PageHelper.startPage(page, limit);
        List<CourseVO> cs = courselService.selCourse(CourseVO);
    ...
    @ResponseBody
    public Object update(CourseVO CourseVO){
         int update = courselService.update(CourseVO);
        if(update==1){
            return "修改成功";
        }else{
            return "修改失败";
    ...
    @ResponseBody
    public Object addcou(CourseVO CourseVO){
        int addcou = courselService.addcou(CourseVO);
        if(addcou==1){
            return "添加成功";
        }else{
            return "添加失败";
